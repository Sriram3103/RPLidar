...
def init_flood_fill(center_point, obstacle_points, xy_points, min_coord,
    ...
    for (x, y) in zip(ox, oy):
        ...
        free_area = midpoint((prev_ix, prev_iy), (ix, iy))
        for fa in free_area:
...
def generate_ray_casting_grid_map(ox, oy, xy_resolution, breshen=True):
    ...
    """
    # In the main function, after generating the occupancy map

    lidar_x = 0.0  # Assuming the LIDAR is at the origin
    lidar_y = 0.0

    # Add these lines to plot the LIDAR location on both subplots
    plt.subplot(121)
    plt.plot(lidar_x, lidar_y, "g*", markersize=10)  # Plot LIDAR as a green star

    plt.subplot(122)
    plt.plot(lidar_x, lidar_y, "g*", markersize=10)  # Plot LIDAR on occupancy map
    ...
    if breshen:
        for (x, y) in zip(ox, oy):
            ...
            # y coordinate of the the occupied area
            iy = int(round((y - min_y) / xy_resolution))
            laser_beams = midpoint((center_x, center_y), (
